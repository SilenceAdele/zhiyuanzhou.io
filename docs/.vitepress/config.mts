...
const teekConfig = defineTeekConfig({
  ...
  blogger: {
    // 博主信息，显示在首页侧边栏
    avatar: "/img/hano.jpg",  //侧边栏个人头像
    shape: "circle-rotate", // 头像风格：square 为方形头像，circle 为圆形头像，circle-rotate 可支持鼠标悬停旋转
    name: "zhiyuan.zhou", // 侧边栏个人昵称
    slogan: "Cloud Observability Engineer", // 侧边栏个人座右铭
    circleBgImg: "https://cdn.jsdelivr.net/gh/SilenceAdele/zhiyuanzhou.io@master/docs/public/img/imgaeBed/bloggerInfoBgi.webp", // 侧边栏个人头像圆形背景图
...
// 布蒜子统计分析
  ...
  riskLink: {
    ...
    whitelist: ["https://github.com/SilenceAdele", "https://blog.zhiyuanzhou.online/", "https://vp.teek.top/", "http://notes.teek.top","https://onedayxyy.cn/", /https:\/\/github.com/, /https:\/\/giee.com/], // 白名单，匹配到的链接不提示风险
  ...
  //            <p>版权：本博客所有文章除特别声明外，均采用<a href="http://www.suncai.net/PubLicense/CCBY40.html" target="_blank" rel="noopener noreferrer">CCBY-NC-SA4.O</a>许可协议。转载请注明来自<a href="https://onedayxyy.cn/" target="_blank" rel="noopener noreferrer">zhiyuan.zhou\'s Blog</a></p>
  //           `,
  //   };
  // },
...
export default defineConfig({
  ...
  extends: teekConfig,
  title: "zhiyuan.zhou's Blog", //左上角网站名称
  ...
  sitemap: {
    hostname: "https://blog.zhiyuanzhou.online",
  ...
  themeConfig: {
    ...
    search: {
      ...
      options: {
        appId: 'NW2LBSQ16W',
        apiKey: '95b149278d5ec08b5c67c7281690213a',
        indexName: 'zhiyuanzhou_blog_online',
     }
    },
    ...
    editLink: {
      ...
      pattern: "https://github.com/SilenceAdele",
    },
  },
      ...
      AutoFrontmatter({
          ...
          transform: (frontMatter: Record<string, any>, fileInfo: FileInfo) => {
              ...
              const rules: TransformRule[] = [
                  { folderName: "01.云原生", prefix: "/kubernetes/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "02.云原生可观测", prefix: "/observability/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "03.云存储", prefix: "/cloud-storage/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "04.中间件", prefix: "/middleware/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "05.操作系统", prefix: "/operating-system/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "06.计算机网络", prefix: "/computer-network/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "07.计算机编程", prefix: "/coding/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "08.故障排查", prefix: "/troubleshooting/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "09.折腾日记", prefix: "/toss-note/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "10.分享", prefix: "/share/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "11.心情", prefix: "/mood/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
                  { folderName: "12.关于我", prefix: "/about/$uuid5", removeLevel: 99}, // 清空前缀并且添加前缀使用随机数
              ];
              // 应用规则转换
